...
// const audio = "https://api.dictionaryapi.dev/media/pronunciations/en/";
...
function play(audio_url) {
    ...
    // audio = srcElement.previousElementSibling;
    // audio.play();
}
...
function phonetic_template(data) {
    ...
    let phoneticHTML =
        `<div class="phonetic">` +
        `<p class="phonetic_text">` +
        // `<a class="phonetic_link" href="${data.audio}" target="_blank"> <img class="phonetic_icon" src="sound.png" /> ${data.text}</a>` +
        // `<audio class="phonetic_link" src="${audio}${data.audio}" target="_blank"> <img class="phonetic_icon" src="sound.png" /> ${data.text}</audio>` +
        // `<audio src="${data.audio}">` +
        // `Your browser does not support the` +
        // `<code>audio</code> element.` +
        // `</audio>` +
        `<img class="phonetic_icon" src="./images/sound.png" onclick="play('${data.audio}')"/>` +
        `${data.text}` +
        `</p>` +
        `</div>`;
    console.log(data);
